...
class Dispatcher:
    ...
    async def execute(self, parts: list[bytes]) -> tuple[bytes, bool]:
        ...
        arity = expected[name]
        if (
            (arity is not None and len(args) != arity)
            or (name in {"DEL", "EXISTS"} and not args)
        ...
        if name in {"INCR", "DECR"}:
            ...
            if self.aof:
                self.aof.append({"op": name, "key": args[0]})
            return integer(result[0]), False
        if name == "SET":
            return await self._set(args), False
        if name == "EXPIRE":
            return await self._expire(args), False
        if name == "TTL":
            return integer(await self.store.ttl(args[0])), False
        if name == "KEYS":
            return array(await self.store.keys(args[0])), False
