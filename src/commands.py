...
class Dispatcher:
    ...
    def _arity(self, command: str, args: list[bytes], minimum: int, maximum: int | None = None) -> bytes | None:
    ...
    async def dispatch(self, parts: list[bytes]) -> tuple[bytes, bool]:
        ...
        if command in ("DEL", "EXISTS"):
            ...
            if command == "DEL":
                ...
                if count: self.aof.append({"op": "DEL", "key": keys[0]})
            else:
        ...
        if command in ("INCR", "DECR"):
            bad = self._arity(name, args, 1, 1)
            if bad: return bad, False
            try: value = await self.store.increment(args[0].decode(), 1 if command == "INCR" else -1)
        ...
        if command == "EXPIRE":
            ...
            if result and seconds > 0: self.aof.append({"op": "EXPIRE", "key": args[0].decode(), "expire_at": time.time() + seconds})
