...
5. **Standard Library `unittest` Mandate:** All unit and integration tests MUST use Python's built-in `unittest` framework (`unittest.TestCase`, `unittest.IsolatedAsyncioTestCase`, `unittest.mock`). Third-party test runners (`pytest`, `nose`, `tox`) are strictly prohibited.
6. **Strict Typing & Zero Linter Warnings:** Full PEP 585 built-in generic collections (`dict`, `list`, `tuple`, `set`). Must pass `ruff check src tests` and `mypy --strict src` with zero findings.
...
```
pyedis/
├── pyproject.toml            # PEP 621 metadata; deps, [tool.mypy], [tool.ruff]
...
│   ├── unit/
│   │   ├── __init__.py
│   │   ├── test_store.py     # Store operations with injected deterministic mock clock
│   │   ├── test_resp.py      # RESP frame encoding, streaming chunk reassembly, pipelined buffers
│   │   ├── test_commands.py  # Arity, unknown commands, case-insensitivity, NX/XX, EX/PX
│   │   └── test_persistence.py  # AOF round-trip: write, absolute TTL replay, corrupt line recovery
│   ├── integration/
│   │   ├── __init__.py
│   │   └── test_server.py    # Live TCP socket tests via redis-py & raw sockets (pipelines, concurrency)
│   └── e2e/
...
- `make e2e` → `docker compose -f docker-compose.e2e.yml up --build --exit-code-from test-runner-e2e` (or host `./tests/e2e/run_tests.sh`).
...
- **`test_resp.py`:** Tests encoding and decoding of all RESP types, partial chunk reassembly, pipelined buffers, inline commands, and binary safety.
- **`test_store.py`:** Tests set, get, del, incr, decr, ttl, active sweep, and lazy eviction using an injected mock clock.
- **`test_commands.py`:** Tests arity checking, unknown commands, case-insensitivity, syntax errors, `NX`/`XX`, and `EX`/`PX`.
- **`test_persistence.py`:** Tests AOF append, startup replay, absolute expiration replay, corrupted trailing line tolerance, and `FLUSHALL` truncation.

### 8.2 Integration Tests (`tests/integration/`)
- **`test_server.py`:** Spins up a live `pyedis` TCP server on an ephemeral port.
